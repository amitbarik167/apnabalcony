...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.msgService.getCartItemsForQtyDisplay().subscribe((product: any) => {
      this.addProductCartQty(product)
    })

    this.msgService.getTemplateCartItemsForQtyDisplay().subscribe((template: any) => {
      this.addTemplateCartQty(template)
    })
    ...
    this.msgService.getRemoveTemplateItemFromCart().subscribe((id: any) => {
      this.cartItems = this.cartItems.filter((item: any) => item.templateId !== id);
      this.cartQty = 0
      for (let i in this.cartItems) {
        this.cartQty = this.cartQty + this.cartItems[i].qty
      }
  ...
  addTemplateCartQty(template: any) {
